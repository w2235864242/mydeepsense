...
FEATURE_DIM = SEPCTURAL_SAMPLES*7*2
mag_dim = 2*3*SEPCTURAL_SAMPLES
gps_dim = 2*2*SEPCTURAL_SAMPLES
...
def read_audio_csv(filename_queue):
	...
	defaultVal = [[0.] for idx in range(WIDE*FEATURE_DIM + OUT_DIM)]#20*5s data
	...
	labels = fileData[WIDE*FEATURE_DIM:]   #zui hou 3 ge
...
def deepSense(inputs, train, reuse=False, name='deepSense'):
	with tf.variable_scope(name, reuse=reuse) as scope:
		...
		avgNum = tf.reduce_sum(mask, reduction_indices=1) #(BATCH_SIZE, INTER_DIM)         # meige yuan su shi 20 huozhe 0 
		...
		mag_inputs = tf.slice(sensor_inputs,[0,0,0],[BATCH_SIZE,WIDE,mag_dim])  #tf.slice(input_, begin, size, name=None)
		gps_inputs = tf.slice(sensor_inputs,[0,0,mag_dim],[BATCH_SIZE,WIDE,gps_dim])
		light_inputs = tf.slice(sensor_inputs,[0,0,mag_dim+gps_dim],[BATCH_SIZE,WIDE,light_dim])
		pressure_inputs = tf.slice(sensor_inputs,[0,0,mag_dim+gps_dim+light_dim],[BATCH_SIZE,WIDE,pressure_dim])
		...
		gps_conv1 = layers.convolution2d(gps_inputs, CONV_NUM, kernel_size=[1, 2*3*CONV_LEN],
		...
		light_conv1 = layers.convolution2d(light_inputs, CONV_NUM, kernel_size=[1, 2 * 3 * CONV_LEN],
		...
		light_conv_out = tf.reshape(light_conv3,
								  [light_conv3_shape[0], light_conv3_shape[1], 1, light_conv3_shape[2], light_conv3_shape[3]])




		pressure_conv1 = layers.convolution2d(pressure_inputs, CONV_NUM, kernel_size=[1, 2 * 3 * CONV_LEN],
										 stride=[1, 2 * 3], padding='VALID', activation_fn=None, data_format='NHWC',
		...
		pressure_conv_out = tf.reshape(pressure_conv3,
								  [pressure_conv3_shape[0], pressure_conv3_shape[1], 1, pressure_conv3_shape[2], pressure_conv3_shape[3]])



		sensor_conv_in = tf.concat([mag_conv_out, gps_conv_out, light_conv_out, pressure_conv_out], 2)
		...
		sum_cell_out = tf.reduce_sum(cell_output*mask, axis=1, keep_dims=False)   # 64*120 meige yuansu doushi 20ge chuangkou xiangjia
		avg_cell_out = sum_cell_out/avgNum                                        # 64*120  chu 64*120 dui yingyuansu xiangchu  avgnum zhong meige yuansu shi 20 huo 0 ,suoyi shi chu 20,yejiushi (T)
