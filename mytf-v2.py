	...
	with tf.variable_scope(name, reuse=reuse):
		...
		with tf.variable_scope("light"):
			...
			light_conv_out = tf.reshape(light_conv3,
									  [light_conv3decay_steps_shape[0], light_conv3_shape[1], light_conv3_shape[2], 1, light_conv3_shape[3]])
	...
	with tf.Graph().as_default():
		...
		with tf.name_scope('train'):
			discOptimizer = tf.train.AdamOptimizer(
					...
					beta1=0.5,
					beta2=0.9
				).minimize(l2_loss, var_list=t_vars)
		...
		with tf.Session() as sess:
			...
			for iteration in xrange(TOTAL_ITER_NUM):
				...
				# 	train_status: True
				# 	})
				_, lossV, train_accuracy = sess.run([discOptimizer, loss, accuracy])
				if iteration % 10 == 0:
